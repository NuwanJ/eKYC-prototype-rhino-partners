...
const side_data = [
  {
    name: "front",
    description:
      "Show the photo page of your photo id (Passport or NIC) to the camera",
  },
...
export default ({ setTabStatus, documentDetails, setDocumentDetails }) => {
  ...
  const [alertMessage, setAlertMessage] = useState(
    "You will be asked to display the front and back of photo IDs.  "
  );
  ...
  const updateDocumentType = (detectedText) => {
    let docType = "Unknown";
    if (_.includes(_.lowerCase(detectedText), "passport")) {
      docType = "Passport";
    } else if (_.includes(_.lowerCase(detectedText), "driving")) {
    ...
    if (faceDetectResponse.$response.error) {
      ...
      Jimp.read(imageBuffer, (err, image) => {
        if (err) throw err;
          ...
          image
            .crop(
              image.bitmap.width * imageBounds.Left - 15,
              image.bitmap.height * imageBounds.Top - 15,
              image.bitmap.width * imageBounds.Width + 30,
              image.bitmap.height * imageBounds.Height + 30
            )
            .quality(100)
  ...
  const filterTextBYDocumentType = (textMap, docType) => {
    let filteredMap = _.filter(textMap, function (word) {
      ...
      return true; // word !== "INDIAN" && word !== "M";
    });
  ...
  const captureTextDetails = async (image) => {
    ...
    let textDetectResponse = await rekognition
      .detectText(textDetectParams)
      .promise();
    var allText;
    if (textDetectResponse.$response.error) {
      ...
      let detectedLines = _.filter(
        textDetectResponse.TextDetections,
        function (item) {
    ...
    let detectEntitiesResponse = await comprehend
      .detectEntities(comprehendParams)
      .promise();
    if (detectEntitiesResponse.$response.error) {
      ...
      let sortedEntities = _.reverse(
        _.sortBy(detectEntitiesResponse.Entities, (entity) => {
          return entity.Score;
        })
      );
      ...
      let filteredEntities = _.filter(
        sortedEntities,
        (entity) => entity.Score > 0.6
      );

      let personEntity = _.find(
        filteredEntities,
    ...
    <>
      <Card>
        <Card.Header>
          {alertMessage}
          {!showProgress && (
            <Button variant="primary" onClick={start_test}>
              Start
            </Button>
          )}
          {progressValue === 5 && (
            <Button variant="primary" onClick={requestSide}>
              Validate
            </Button>
          )}
          {progressValue === 100 && (
