...
export default ({ setTabStatus, setLiveTestDetails }) => {
  ...
  const [alertMessage, setAlertMessage] = useState(
    "You will be asked to do a series of random gestures which will enable us to detect a live feed.  "
  );
  ...
  const validateGesture = (gesture, data) => {
    ...
    if (data.length === 0) {
      ...
      return {
        result: false,
        message: "Could not recognize a face. Try again ",
      };
    }
    ...
    if (gesture === "smile") {
      if (data.FaceDetails[0].Smile.Value === true) {
        ...
        return {
          result: false,
          message: "Failed to validate smile. Try again ",
        };
      }
    } else if (gesture === "lookRight") {
      if (data.FaceDetails[0].Pose.Yaw <= -30) {
        ...
        return {
          result: false,
          message: "Failed to validate face turning right. Try again ",
        };
      }
      ...
      if (data.FaceDetails[0].MouthOpen.Value === true) {
        return {
          result: true,
          message: "You can successfully completed Liveness checks !! ",
        };
      } else {
        return {
          result: false,
          message: "Failed to validate open mouth. Try again ",
        };
      }
    }
      ...
      <Card>
        <Card.Header>
          ...
          {showProgress && progressValue < 100 && (
            <Button variant="primary" onClick={requestGesture}>
              Validate
            </Button>
          )}
          {progressValue === 100 && (
